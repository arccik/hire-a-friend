import Image from "next/image";
import { api } from "~/utils/api";

export default function DashboardUsersTable() {
  const { data: usersData, status } = api.user.lastFiveUsers.useQuery();
  if (status === "loading") return <p>Loading...</p>;
  return (
    <div id="last-users">
      <h1 className="py-4 font-bold uppercase">Last 24h users</h1>
      <div className="overflow-x-scroll">
        <table className="w-full whitespace-nowrap">
          <thead className="bg-black/60">
            <tr>
              <th className="rounded-l-lg px-2 py-3 text-left">Name</th>
              <th className="px-2 py-3 text-left">Email</th>
              <th className="px-2 py-3 text-left">Group</th>
              <th className="px-2 py-3 text-left">Status</th>
              <th className="rounded-r-lg px-2 py-3 text-left">Actions</th>
            </tr>
          </thead>
          <tbody>
            {usersData?.map((user) => (
              <tr key={user.id} className="border-b border-gray-700">
                <td className="px-2 py-3 font-bold">
                  <div className="inline-flex items-center space-x-3">
                    {user.image && (
                      <span>
                        <Image
                          width={32}
                          height={32}
                          className="h-8 w-8 rounded-full"
                          src={user.image}
                          alt=""
                        />
                      </span>
                    )}
                    <span>{user.name}</span>
                  </div>
                </td>
                <td className="px-2 py-3">{user.email}</td>
                <td className="px-2 py-3">{user.userType}</td>
                <td className="px-2 py-3">{user.status}</td>
                <td className="px-2 py-3">
                  <div className="inline-flex items-center space-x-3">
                    <a href="" title="Edit" className="hover:text-white">
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        fill="none"
                        viewBox="0 0 24 24"
                        strokeWidth="1.5"
                        stroke="currentColor"
                        className="h-5 w-5"
                      >
                        <path
                          strokeLinecap="round"
                          strokeLinejoin="round"
                          d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10"
                        />
                      </svg>
                    </a>
                    <a
                      href=""
                      title="Edit password"
                      className="hover:text-white"
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        fill="none"
                        viewBox="0 0 24 24"
                        strokeWidth="1.5"
                        stroke="currentColor"
                        className="h-5 w-5"
                      >
                        <path
                          strokeLinecap="round"
                          strokeLinejoin="round"
                          d="M16.5 10.5V6.75a4.5 4.5 0 10-9 0v3.75m-.75 11.25h10.5a2.25 2.25 0 002.25-2.25v-6.75a2.25 2.25 0 00-2.25-2.25H6.75a2.25 2.25 0 00-2.25 2.25v6.75a2.25 2.25 0 002.25 2.25z"
                        />
                      </svg>
                    </a>
                    <a
                      href=""
                      title="Suspend user"
                      className="hover:text-white"
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        fill="none"
                        viewBox="0 0 24 24"
                        strokeWidth="1.5"
                        stroke="currentColor"
                        className="h-5 w-5"
                      >
                        <path
                          strokeLinecap="round"
                          strokeLinejoin="round"
                          d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636"
                        />
                      </svg>
                    </a>
                  </div>
                </td>
              </tr>
            ))}
          </tbody>
        </table>
      </div>
    </div>
  );
}
