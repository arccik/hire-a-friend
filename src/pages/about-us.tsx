export default function AboutPage() {
  return (
    <div>
      <div className="bg-white py-16">
        <div className="container m-auto px-6 text-gray-600 md:px-12 xl:px-6">
          <div className="space-y-6 md:flex md:gap-6 md:space-y-0 lg:items-center lg:gap-12">
            <div className="md:5/12 lg:w-5/12">
              <img
                src="https://tailus.io/sources/blocks/left-image/preview/images/startup.png"
                alt="image"
                loading="lazy"
                width=""
                height=""
              />
            </div>
            <div className="md:7/12 lg:w-6/12">
              <h2 className="text-2xl font-bold text-gray-900 md:text-4xl">
                Rent My Time
              </h2>
              <p>
                Connects you with local companions across the United Kingdom.
                Whether you want to enjoy activities, get advice, or find a
                friendly face for an event, we&#39;ve got you covered.
              </p>
              <p>
                How does RentMyTime.co.uk work? Simply search for companions in
                your UK location. Browse profiles, view photos, and learn about
                their interests for free. To connect, register as a member with
                a small fee.
              </p>
              <p>How do people use RentMyTime.co.uk?</p>
              <p>
                Discover local insights with a companion. Never attend events
                alone. Enjoy movies, dining, or outings together. Learn new
                skills or hobbies. Connect with people from diverse backgrounds.
                Find dinner or sports game companions. Get a workout partner.
                Seek personal advice and fresh perspectives. What are the costs?
                Most companions start at just £10 per hour, with room for
                negotiation, exclusive to the UK.
              </p>
              <p>
                How can I contact companions? Message or call them securely as a
                registered member.
              </p>
              <p>
                Ready to Begin? Browse companion profiles for free and find your
                ideal friend. Click the green button below to start your search
                on RentMyTime.co.uk.
              </p>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}
